...
class YesterdayBullishDailyCandle(PatternAnalyser):
    ...
    def __init__(self, hit_scanner_date: datetime.date, daily_df: pd.DataFrame, ticker_to_contract_info_dict: dict, ticker_to_offerings_news_dict: dict, ticker_to_financial_data_dict: dict, discord_client, sqlite_connector):
        ...
        if len(yesterday_bullish_daily_candle_ticker_list) > 0:
            for ticker in yesterday_bullish_daily_candle_ticker_list:
                with pd.option_context('display.max_rows', None,
                                    ...
                                    'display.precision', 3):
                    logger.log_debug_msg(f'{ticker} Bullish Daily Candle Boolean Dataframe: {green_candle_boolean_df.loc[:, idx[[ticker], :]]}')
                    logger.log_debug_msg(f'{ticker} Yesterday Bullish Daily Candle Full Dataframe: {self.__historical_data_df.loc[:, idx[[ticker], :]]}')
                ...
                close = self.__historical_data_df.loc[self.__hit_scanner_date, (ticker, Indicator.CLOSE.value)]
                close_pct = self.__historical_data_df.loc[self.__hit_scanner_date, (ticker, CustomisedIndicator.CLOSE_CHANGE.value)]
                volume = self.__historical_data_df.loc[self.__hit_scanner_date, (ticker, Indicator.VOLUME.value)]
