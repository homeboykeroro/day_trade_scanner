...
class Scanner:
    ...
    def __analyse_yesterday_top_gainer(self):
        try:
    ...
    def __analyse_intra_day_top_gainer(self) -> None:
        try:
            loop = asyncio.get_event_loop()
        except RuntimeError:
            loop = asyncio.new_event_loop()
        ...
        one_minute_candle_df = self.__retrieve_intra_day_minute_candle(contract_list, BarSize.ONE_MINUTE, loop)
    ...
    def __analyse_intra_day_top_loser(self) -> None:
        try:
            loop = asyncio.get_event_loop()
        except RuntimeError:
            loop = asyncio.new_event_loop()
    ...
    def __retrieve_intra_day_minute_candle(self, contract_list: list, bar_size: BarSize, loop) -> pd.DataFrame:
        ...
        #historical_data_interval_in_minute < 1
        if False:
    ...
    def __retrieve_yesterday_minute_candle(self, contract_list: list, bar_size: BarSize, outside_rth: bool = True, loop = None):
    ...
    def __get_daily_candle(self, contract_list: list, offset_day: int, outside_rth: bool = False, candle_retrieval_end_datetime: datetime.datetime = None, loop = None) -> pd.DataFrame:
        candle_request_contract_list = []
        contract_ticker_list = [contract['symbol'] for contract in contract_list]
        ...
        if candle_request_contract_list:    
            ...
            candle_df = self.__ib_connector.get_historical_candle_df(contract_list=candle_request_contract_list, 
                                                                     ...
                                                                     outside_rth=outside_rth_str, 
                                                                     candle_retrieval_end_datetime=candle_retrieval_end_datetime, loop=loop)
            if candle_df is not None and not candle_df.empty:
                complete_df = append_customised_indicator(candle_df)
