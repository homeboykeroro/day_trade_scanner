...
class DayTradeScanner():
        ...
        while True: 
            if self.__is_scanner_ready:
                ...
                try:
                    ...
                    logger.log_debug_msg(f'Get completed data time taken: {time.time() - scan_start_time}') 
                                    ...
                                    for message in analysed_result_message_list:
                                        ...
                                        if not is_message_sent:
                                            if message.display_message or message.embed:
                                                logger.log_debug_msg(f'Send display message: {message.display_message}', with_std_out=True)
                                            ...
                                            if message.read_out_message:
                                                logger.log_debug_msg(f'Send readout message: {message.read_out_message}', with_std_out=True)
    ...
    def __wait_till_next_scan(self, scan_start_time):
        ...
        logger.log_debug_msg(f'Scan time taken in wait unitl: {scan_time}', with_std_out=True)
