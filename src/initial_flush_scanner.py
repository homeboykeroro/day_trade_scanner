...
threading.current_thread().name = "InitialFlushScannerThread"
...
# Chatbot Token
CHATBOT_TOKEN = os.environ['DISCORD_INITIAL_FLUSH_CHATBOT_TOKEN']

# Top loser filter parameter
MAX_NO_OF_SCANNER_RESULT = get_config('TOP_LOSER_SCANNER', 'MAX_NO_OF_SCANNER_RESULT')
MIN_PRICE = get_config('TOP_LOSER_SCANNER', 'MIN_PRICE')
PERCENT_CHANGE_PARAM = get_config('TOP_LOSER_SCANNER', 'PERCENT_CHANGE_PARAM')
MIN_USD_VOLUME = get_config('TOP_LOSER_SCANNER', 'MIN_USD_VOLUME')
MAX_MARKET_CAP = get_config('TOP_LOSER_SCANNER', 'MAX_MARKET_CAP')
    ...
    while True: 
        ...
        if start_scan:
            ...
            if idle_message:
                ...
                idle_message = 'Initial flush scanner is idle until valid trading weekday and time'

            time.sleep(SCANNER_IDLE_REFRESH_INTERVAL)
    
    logger.log_debug_msg('start sending testing message')
    discord_client.send_message_by_list_with_response([DiscordMessage(content='Initial Flush Scanner Starts')], channel_type=DiscordChannel.TEXT_TO_SPEECH, with_text_to_speech=True)
    logger.log_debug_msg('flush scan completed')
